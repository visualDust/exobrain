...
def models_add(ctx: click.Context) -> None:
    ...
    provider_name = Prompt.ask(
        "How would you call it? (e.g., custom-openai)",
    ...
    while True:
        model_name_input = Prompt.ask(
            f"Model #{model_num} (or press Enter to finish)",
            default=""
        ).strip()
    ...
    set_defaults = Confirm.ask(
        "Would you like to set default temperature and/or max_tokens?",
        default=False
    )
    ...
    test_connection = Confirm.ask(
        "Would you like to test the connection to this provider?",
        default=True
    )
    ...
    if test_connection:
        ...
        try:
            ...
            provider = factory.get_provider(test_model)
